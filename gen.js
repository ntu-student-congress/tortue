...
fs.readdir("laws", function(err, law_cats) {
	var cats_latex = law_cats.map(function(law_cat) {
		...
		var cat_latex = '\\newpage\\vspace*{\\fill}\\begin{center} \\fontsize{80pt}{100pt} \\fontnoto ' + law_cat.split('_')[1] + '\\end{center}\\vspace*{\\fill}\n';
		...
		var laws_latex = laws.map(function(law) {
			...
			var law_latex = "\\cleardoublepage \\section*{" + law_data[0] + "}\n\\addcontentsline{toc}{section}{" + law_data[0] + "}"; // 法名
			...
			var formatted_lines = law_data[2].split("\n").map(function(element, index, array) {
				...
				switch(element.search(/\S/) / 2){
					case 1: // 條
						return "\\vspace{0.15em}\\textbf{" + text.replace('【', '【{\\fontml ').replace('】', '}】') + "}\\\\\\vspace{0em}";
					case 2: // 項
						return "\\begingroup\\raggedright\\leftskip=0em\\hspace{2em}\\vspace{0em}" + text + " \\par\\endgroup\\par\\vspace{0em}";
					case 3: // 款
